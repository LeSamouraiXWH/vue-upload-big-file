    ...
    <div class="total-progress">
      <div class="btns">
        <el-button-group>
          ...
          <el-button :disabled="pauseDisabled" @click="handlePause"><i class="el-icon-video-pause el-icon--left" size="mini"></i>全部暂停</el-button>
          <el-button :disabled="resumeDisabled" @click="handleResume"><i class="el-icon-video-play el-icon--left" size="mini"></i>全部开始</el-button>
          <el-button :disabled="clearDisabled" @click="clearFiles"><i class="el-icon-video-play el-icon--left" size="mini"></i>全部取消</el-button>
        </el-button-group>
            ...
            <div class="progress-box">
              <div class="list-item">
                ...
                <div v-else class="item-progress">
                  <span>文件进度：</span><el-progress :percentage="item.uploadProgress" />
                </div>
                ...
                <div class="setting">
                  <el-button :disabled="['wait', 'success', 'secondPass','pause'].includes(item.status)" @click.stop="handlePauseItem"><i class="el-icon-video-pause el-icon--left" size="mini"></i>暂停</el-button>
                  <el-button :disabled="!['pause'].includes(item.status)" @click.stop="handleResumeItem(index)"><i class="el-icon-video-play el-icon--left" size="mini"></i>恢复</el-button>
                  <el-button @click.stop="clearFilesItem(index)"><i class="el-icon-video-play el-icon--left" size="mini"></i>取消</el-button>
                </div>
              </div>
            </div>
          </template>
...
import {handleFileChangeUpload} from '@/utils/upload'
...
export default {
  ...
  methods: {
    ...
    async handleUpload() {
      console.log('handleUpload -> this.uploadFiles', this.uploadFiles.length);
      ...
      for (let i = 0; i < filesArr.length; i++) {
        ...
        const fileChunkList = await this.createFileChunk(filesArr[i]);
    ...
    // 创建文件切片
    async createFileChunk(file, size = chunkSize) {
      const fileChunkList = [];
      var count = 0;
    ...
    clearFilesItem(index) {
      this.uploadFiles.splice(index,1)
      // this.$set(this.uploadFiles, index, this.uploadFiles[index]);

      console.log('delete current item', this.uploadFiles)
      if(this.uploadFiles.length === 0){
        this.clearFiles()
        return
      }
      console.log(this.uploadFiles)
      if(this.uploadFiles[fileIndex].uploadProgress > 0){
        this.handlePause();
        this.handleResume()
        }
    },
